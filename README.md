...
> for itself — and get an answer built on live model prices, a three-layer cost model, and
...
[![Prices](https://img.shields.io/badge/prices-AI%20Pricing%20Hub-ACE849?labelColor=2C2C2C)](https://aipricinghub.optimnow.io)
...
and batch discounts entirely, and quietly skip the orchestration, retrieval and monitoring
that make up most of the real bill. The answer looks confident. The arithmetic underneath
is usually wrong, and there is no way to check it.
...
- **Layer 2 — Harness.** Orchestration, retrieval, tool APIs, logging, guardrails, egress,
  storage. The part everyone forgets, and often the larger half.
- **Layer 3 — Business value.** Four archetypes: cost displacement, revenue uplift,
  retention uplift, premium monetization — with a realization rate, because not every
  technically-successful output turns into money.

Every figure it reports comes with the price date it was computed from.
...
| **`load-preset`** | "What are sensible defaults for this use case?" Returns one of 11 preset scenarios without computing anything. |
| **`sensitivity-analysis`** | "What breaks this business case?" Impact ranking of volume, realization rate, cost and value at ±20%. |

All four are read-only and take no credentials. Nothing you send is stored.
...
[web calculator](https://airoicalculator.optimnow.io), so a conversation can end with
"open this and adjust it" rather than a dead end.

---

## Where the numbers come from

**Prices** are fetched live from the [AI Pricing Hub](https://aipricinghub.optimnow.io)
catalog, which tracks 250+ models and refreshes daily from OpenRouter. If the hub is
unreachable the server falls back to an embedded snapshot rather than failing the call, and
says which it used — so a figure is never reported without its provenance.
...
with `--from <path>` or `ROI_CALCULATOR_PATH`.
...
| [**AI Pricing Hub**](https://aipricinghub.optimnow.io) | Compare what 250+ models cost per request, with caching and batch factored in. |
